...
describe("PwwWorkbenchWeb 非发布 fixture 示例", () => {
  ...
  it("只从 phoenix-wing 公共入口消费组件、类型和样式", () => {
    ...
    expect(pwwReadFile("src/fixture/PwwFixtureWorkbenchStore.ts"))
  ...
  it("不连接 Router、产品 API 或偏好持久化", () => {
    const sources = pwwReadSourceTree("src");

    expect(sources).not.toMatch(/vue-router|\bfetch\s*\(|\baxios\b|localStorage|sessionStorage/u);
    ...
    expect(sources).toContain("PnwWorkbenchShell");
    expect(sources).toContain('<details class="pww-display-settings-extra" open>');
