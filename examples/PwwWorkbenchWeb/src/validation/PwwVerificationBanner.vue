...
<template>
  <aside class="pww-verification-banner" aria-label="验证来源" :data-source="source.mode">
    <strong>{{ source.mode === "development" ? "开发验证 · 本地构建" : "Registry 验证 · npm 正式包" }}</strong>
    ...
    <small :title="source.checkedAt">{{ source.mode === 'development' ? "启动快照；不代表 Registry 验收" : "已安装正式包；不代表全部测试通过" }}</small>
