...
4. **[进行中：共享编辑抽屉 / Editor Host]** Wing 已实现并构建 `PnwEditorDrawerHost`、关闭守卫契约与单元测试；下一步是发布包含该能力的新版本，由 Phoenix Function 与 Phoenix Open Issue 两个真实消费者完成 Registry 验证后再宣布稳定公共 API。
