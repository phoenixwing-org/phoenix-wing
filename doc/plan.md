...
4. **[进行中：0.5.1 已发布 / Editor Host 验证]** Wing 已实现并公开发布 `PnwEditorDrawerHost`、关闭守卫契约与单元测试；Phoenix Function 已完成首个本地真消费与两处抽屉浏览器验收。下一步由 Function 与 Phoenix Open Issue 独立完成 Registry 验证后，再宣布稳定公共 API。
   ...
   - [ ] Phoenix Function 把 manifest / lockfile 精确升级到 Registry 0.5.1 并移除 0.4.3 兼容回退。
