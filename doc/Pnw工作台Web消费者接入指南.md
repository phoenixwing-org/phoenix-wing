...
建议消费者在自己的 Pinia store 保存以下可序列化状态：

- `PnwActivityBarPresentation`；
- `expandedNodeIds` 与 `treeCollapsed`；
- `PnwRibbonAppearance`；
...
Open Issue 的独立验证分支采用上述方式：manifest 仍锁定 `phoenix-wing@0.5.1`，`pnpm dev:local-wing` / `pnpm build:local-wing` 在进程内消费并列 Wing `0.5.2`。这是本地候选证据，不是 Registry 验收。
...
8. 类型、测试、构建、文档门禁通过；
9. manifest、lockfile、node_modules 没有本地路径污染；
10. 记录尚无两个真实消费者证明的产品语义，不把它扩成 Wing API。
