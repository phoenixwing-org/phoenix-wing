...
- 默认不在公开源码、示例、文档、提交说明或 npm 制品中写入具体用户、私有消费者名称、仓库地址、本机路径、业务数据或凭据。使用“消费者 A/B”“本地工作台”“领域组件”等中性描述。
- 只有已明确核实开源许可证且相关内容允许公开的消费者，才可保留其身份；可访问的仓库或本地联调成功不等于开源授权。不得公开其账号、私有环境或未经授权的数据。
- 发布前分别检查本次 diff、Git 跟踪文件及实际 tarball。既有消费者的开源依据不明时列为待核对，不以历史文档已出现为公开依据。
- 脱敏不得虚构验收结果；公开材料可保留匿名场景和测试范围。历史提交不自动重写，发现凭据另行处置并通知维护者。

### View Header 插槽迁移（AI 必读）

- 新页面优先使用 `PnwPageHeader` / `PnwPageLayout` 的 `#center`（搜索、筛选、配置）和 `#right`（保存、刷新等业务操作），允许同时使用。
- 不传 `#center` 时不渲染中间容器、不占中间列；不要传空插槽占位。框架浮出/收回由 Wing 独立提供，始终在最右侧，业务插槽不能替代。
- `#actions`、`#help` 暂时保留原行为，仅用于迁移兼容。修改已有消费者页面时尽快统一切换新插槽；不得用内部 `.pnw-head-*` CSS 覆盖模拟新布局。
- 本地候选能力必须先验证、归档并提升到主库；Registry 消费者必须确认安装版本支持新插槽后再迁移，不能把尚未发布的能力当作已发布。
- 详细契约与验收见 `docs/View页面标准.md`。
- 收到“查 Wing UI 标准，参考示例改造”时，先读该文档的快速选型清单，选择页面类型和必要定制项，再核对公开 API 与示例；不让用户重复描述固定页头、框架动作和默认主题。可视化选型面板仍是 TODO，不得声称已有生成器。
- 新控件或页面布局能力必须在 `examples/PwwWorkbenchWeb` 首页“内部 Tab 与浮窗排列”或其可发现的并列验证页展示，并先在 Wing 自身验证，再交给消费者。Header 插槽入口为 `?example=header`；不可仅凭消费者截图代替框架验收。
